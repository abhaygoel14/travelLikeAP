...
export async function getUserById(id) {
  const result =
    await sql`SELECT id, name, email, role, created_at FROM users WHERE id = ${id} LIMIT 1`;
  return result[0];
}
...
export async function getAllUser(req, res) {
  try {
    const users =
      await sql`SELECT id, name, email, role, created_at FROM users ORDER BY id DESC`;
